...
exports.isMatchedProgram = function (rules, program) {
	...
	for (i = 0; i < rules.length; i++) {
		if (exports.programMatchesRule(rules[i], program, fullTitle_norm, detail_norm)) {
			return true;
		}
	}
...
// 単体のルールとのマッチ判定
exports.programMatchesRule = function (rule, program, fullTitle_norm, detail_norm) {
	var i, j, l, m, isFound;
	...
	if (rule.reserve_titles) {
		...
		for (i = 0; i < rule.reserve_titles.length; i++) {
			if (program.fullTitle.match(rule.reserve_titles[i].normalize(nf)) !== null) { isFound = true; }
		}
	...
	if (rule.ignore_titles) {
		for (i = 0; i < rule.ignore_titles.length; i++) {
			if (program.fullTitle.match(rule.ignore_titles[i].normalize(nf)) !== null) { return false; }
		}
	}
	...
	if (rule.reserve_descriptions) {
		...
		for (i = 0; i < rule.reserve_descriptions.length; i++) {
			if (program.detail.match(rule.reserve_descriptions[i].normalize(nf)) !== null) { isFound = true; }
		}
	...
	if (rule.ignore_descriptions) {
		...
		for (i = 0; i < rule.ignore_descriptions.length; i++) {
			if (program.detail.match(rule.ignore_descriptions[i].normalize(nf)) !== null) { return false; }
		}
	}
